require(['jquery'],function($){'use strict';$(window).on('load',function(){var bvReviewLinkTime;function resetAnchorPosition(){var scrollTopPosition=$('header.page-header').size()?$('header.page-header').outerHeight():0;if($('.inside-notice').size()){scrollTopPosition+=$('.inside-notice').outerHeight();}
if($('#bv_reviews').size()){$('html, body').animate({scrollTop:$('#bv_reviews').offset().top-scrollTopPosition},0);}}
